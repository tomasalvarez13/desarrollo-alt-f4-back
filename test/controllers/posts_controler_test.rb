...
class PostsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'crear posts sin data' do
    post '/posts', params: { price: 10, placement: 1, height: 1.0, width: 1.0 }, as: :json,
                   headers: { 'HTTP_AUTHORIZATION' => @authorization }
    assert_response 422
  end
  ...
  test 'edit post' do
    patch "/posts/#{@post.id}",
          params: { price: 100, placement: 1, height: 1.0, width: 1.0, image_url: 'www.url.org' }, as: :json, headers: { 'HTTP_AUTHORIZATION' => @authorization }
    assert_response :success
  end
  ...
  test 'find posts' do
    get '/posts/filter', headers: { 'HTTP_AUTHORIZATION' => @authorization }
    assert_response :success
  end
end
